...
class Hypergraph(BaseHypergraph):
    ...
    def calculate_Xe_mean_of_edge(self, train_node, X):
        # 获取超边的特征矩阵 Xe
        Xe = self.v2e_aggregation(X)

        # 创建一个字典来保存每个节点的超边特征均值
        node_edge_mean_features = {}

        for node in train_node:
            # 获取该节点连接的所有超边的索引
            connected_edges = self.N_e(node)  # 假设有方法 N_e 获取节点连接的超边

            # 获取这些超边的特征，并计算均值
            connected_edge_features = []
            for e_idx in connected_edges:
                edge_feature = Xe[e_idx]
                connected_edge_features.append(edge_feature)

            # 计算所有连接超边的特征的均值
            if connected_edge_features:
                mean_edge_feature = torch.mean(torch.stack(connected_edge_features), dim=0)
            else:
                # 如果没有连接的超边，均值设为0（或其他默认值）
                mean_edge_feature = torch.zeros_like(X[0])  # 假设X是节点特征矩阵，取与其相同的维度

            # 保存节点的超边特征均值
            node_edge_mean_features[node] = mean_edge_feature

        return node_edge_mean_features
    ...
    def calculate_top20_uv_similarity(self, node_pairs, X, l):
        """
        计算输入的节点对的余弦相似度，并返回相似度排序后前l%的节点对。

        Args:
        - node_pairs (list of tuples): 需要计算相似度的节点对，例如 [(u1, v1), (u2, v2), ...]
        - X (torch.Tensor): 节点特征矩阵，其中 X[i] 表示第 i 个节点的特征向量。

        Returns:
        - top_20_percent_pairs (list): 排序后的前20%节点对及其相似度 [(u, v, similarity), ...]
        """
        similarities = []
        ...
        for u, v in node_pairs:
            # 计算节点 u 和节点 v 的余弦相似度
            similarity = self.cosine_similarity(X[u], X[v])
        ...
        # 选择前l%的节点对
        #top_20_percent_count = max(1, int(len(similarities) * l))  # 至少保留一个节点对
        top_20_percent_count = 5
        top_20_percent_pairs = [(u, v) for u, v, _ in similarities[:top_20_percent_count]]
        return top_20_percent_pairs
    ...
    def index_edges_between(hg, node1, node2):
        # 获取每个节点的超边列表
        hyperedges_node1 = hg.N_e(node1).tolist()
        hyperedges_node2 = hg.N_e(node2).tolist()
    ...
    # 更新后的 homogeneity 函数
    def homogeneity(self, e_idx: int):
        # 获取超边 e_idx 中的顶点
        e = self.N_v(e_idx).tolist()
        if len(e) <= 1:
            return 0.0, []
        ...
        # 计算每对顶点之间的边数
        total_edges = 0
        vertex_pair_to_edges = defaultdict(int)  # 用于记录每对顶点共享的超边数

        # 计算每一对顶点共享的超边数
        for u, v in itertools.combinations(e, 2):
            edge_count = self.count_edges_between(u, v)  #该函数可以返回 u 和 v 之间的共享超边数
            total_edges += edge_count
            if edge_count > 0:
                vertex_pair_to_edges[(u, v)] += edge_count

        # 计算同质性
        homogeneity = total_edges / num_pairs if num_pairs > 0 else 0.0

        # 找到共享超边最多的顶点对，只返回第一个
        max_shared_edges = max(vertex_pair_to_edges.values(), default=0)
        max_pairs = []

        # 标志来确保只返回第一个共享边数最多的节点对
        for pair, count in vertex_pair_to_edges.items():
            if count == max_shared_edges:
                max_pairs = [(pair, [e_idx])]  # 只保留第一个匹配的节点对
                break  # 一旦找到第一个，跳出循环

        return homogeneity, max_pairs

    # 更新后的 homogeneities 函数
    def homogeneities(self) -> torch.Tensor:
        # 计算所有超边的同质性和共享超边最多的节点对
        homogeneities = []
        node_pairs = []  # 存储节点对和对应的超边索引

        for e_idx in range(self.num_e):
            homogeneity_value, max_pairs = self.homogeneity(e_idx)
            homogeneities.append(homogeneity_value)

            # 处理 max_pairs，保存每对节点对
            for pair in max_pairs:
                node_pairs.append(pair)

        # 将同质性值转换为张量
        homogeneity_tensor = torch.tensor(homogeneities, dtype=torch.float32)

        # 返回同质性张量和节点对及其超边索引
        return homogeneity_tensor, node_pairs

    def hyperedges_beyond_homogeneity(self, threshold: float = 4.0) -> list[Tensor]:
        # 获取所有超边的同质性
        homogeneities = self.homogeneities()

        # 获取同质性大于阈值的超边索引
        high_homogeneity_indices = [idx for idx, homogeneity in enumerate(homogeneities) if homogeneity > threshold]

        # 根据索引获取对应的超边信息
        edges_to_remove = [self.N_v(idx) for idx in high_homogeneity_indices]
        # print("=============================")
    ...
    def top20_percent_hyperedges_index(self):
        # 获取所有超边的同质性
        homogeneities, max_pair = self.homogeneities()

        # 计算同质性top20的超边数量
        num_hyperedges_to_modify = max(1 , int(len(homogeneities) * 0.3))

        # 按同质性值对超边进行排序，并获取最高20%的超边索引
        sorted_indices = torch.argsort(homogeneities)
        top_20_percent_hyperedge = sorted_indices[:num_hyperedges_to_modify]
        #print(f"同质性最高的超边同质性: {homogeneities[top_20_percent_hyperedge[0]]}")
        # # 确保不会尝试移除不存在的超边
        # if len(top_20_percent_hyperedge) > 0:
        #     # 根据索引获取对应的超边信息
        #     edges_to_remove = [self.N_v(idx.item()) for idx in top_20_percent_hyperedge]
        #
        #     # 使用列表推导式将 tensor 对象转换为列表
        #     edges_to_remove_list = [edge.tolist() for edge in edges_to_remove]
        #
        ...
        top_20_percent_hyperedge = [edge.tolist() for edge in top_20_percent_hyperedge]
        return top_20_percent_hyperedge,max_pair
    ...
    def _e_list_from_graph_kHop(graph: "Graph", k: int, only_kHop: bool = False,) -> List[tuple]:
        ...
        A_history = []
        for _ in range(k - 1):
            A_k = torch.sparse.mm(A_k, A_1)     #A_K用于累计k跳邻居关系
            if not only_kHop:
                A_history.append(A_k.clone())
        if not only_kHop:
            A_k = A_1
            for A_ in A_history:
                A_k = A_k + A_      #此时A_K为完整的k跳邻接矩阵
        e_list = [                  #e_list: 包含所有超边的列表。每个元素是一个元组，表示一条超边，其中包含中心顶点及其 k 跳邻居的集合。
    ...
    @H.setter
    def H(self, H_padone: torch.Tensor) -> None:
        H_vtoe = self.H_v2e
        self.cache["H"] = torch.cat((H_padone, H_vtoe), dim=0)
    ...
    def D_v_of_group(self, group_name: str) -> torch.Tensor:
        ...
        if self.group_cache[group_name].get("D_v") is None:
            ...
            val = w_e[H._indices()[1]] * H._values().to(device = 'cuda') #改了to。cuda
    ...
    def v2e_update(self, X: torch.Tensor, e_weight: Optional[torch.Tensor] = None):
        ...
        Args:
            ``X`` (``torch.Tensor``): 超边特征Hyperedge feature matrix. Size :math:`(|\mathcal{E}|, C)`.
    ...
    def v2v(
        ...
        Args:
            ...
            ``v2e_weight`` (``torch.Tensor``, optional): The weight vec
            ```-tor attached to connections (vertices point to hyepredges). If not specified, the function will use the weights specified in hypergraph construction. Defaults to ``None``.
